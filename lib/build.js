...
function buildElectron(options) {
    ...
    const { name, version, description, author, licence, homepage, dependencies } =
        require(path.join(projectRoot, "package.json"));
    const appMeta = {
        name, version, description, author, licence, homepage, dependencies,
        productName: "Padlock",
        main: "main.js"
    };

    const promise = bundle("dist/electron")
        .then(() => Promise.all([
            copyFiles(".", "dist/electron", [ "package.json" ]),
            copyFiles("electron", "dist/electron", [ "main.js" ])
        ]))
    ...
    if (!options.mac && !options.win && !options.linx) {
        return promise;
    }

    return promise.then(() => builder.build({
